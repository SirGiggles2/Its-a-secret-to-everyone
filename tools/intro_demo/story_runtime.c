/* tools/intro_demo/story_runtime.c
 *
 ...
 * clobbered by the title fade-out. story_runtime_load() restores
...
/* Persistent state across story_runtime_step(). All initialized to 0 (BSS);
 * non-zero initial values are written by story_runtime_load() to keep the
...
unsigned char story_runtime_at_end(void) {
    return s_at_end;
}

void story_runtime_clear_end(void) {
    s_at_end = 0;
}
