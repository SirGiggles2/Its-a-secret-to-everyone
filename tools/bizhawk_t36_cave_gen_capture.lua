-- bizhawk_t36_cave_gen_capture.lua
-- T35 Genesis trace: mirror of bizhawk_t36_cave_nes_capture.lua on the
...
--
-- Outputs:
--   builds/reports/t36_cave_gen_capture.txt
--   builds/reports/t36_cave_gen_capture.json
--   builds/reports/t36_cave_gen_capture.png
...
local OUT_TXT  = repo_path("builds\\reports\\t36_cave_gen_capture.txt")
...
local FLOW_T36_STABILIZE   = "T36_STABILIZE"
local FLOW_T36_CAPTURE     = "T36_CAPTURE"
...
for frame = 1, MAX_FRAMES do
    ...
    if flow_state == FLOW_REPLAY then
        ...
        -- see Mode5/room77 + Link stable. At that point switch to T36_STABILIZE
        ...
        if mode == 0x05 and room_id == TARGET_ROOM_ID then
            ...
            set_flow(FLOW_T36_STABILIZE, frame, "replay hit gameplay")
        end
    ...
    elseif flow_state == FLOW_FS1_START_GAME then
        if mode ~= 0x01 then
            set_flow(FLOW_T36_STABILIZE, frame, "left Mode1 (gameplay starting)")
        else
            local target = 0x00
    ...
    elseif flow_state == FLOW_T36_STABILIZE then
        if mode == 0x05 and room_id == TARGET_ROOM_ID then
            if not CAPTURE.reached_mode5 then
                CAPTURE.reached_mode5 = true
                CAPTURE.reached_mode5_frame = frame
    ...
    elseif flow_state == FLOW_T36_CAPTURE then
    ...
    local pad
    if flow_state == FLOW_T36_CAPTURE then
...
local verdict = ok_capture and "T36_GEN_CAPTURE: OK" or "T36_GEN_CAPTURE: FAIL"
record(verdict)
