...
local OUT_PATH = OUT_DIR .. "bizhawk_phase3_left_navigation_probe.txt"
...
local function main()
    ...
    if settled_frame == nil then
        error("room never settled after walking left")
    end
    if settled_room ~= 0x0076 then
        error(string.format("expected left-adjacent room 0076, got %04X", settled_room))
    end
    if settled_x < 230 then
        error(string.format("expected landing near right edge, got x=%d", settled_x))
    end

    log(string.format("transition settled left after %d frames", settled_frame))
    log("WHAT IF Phase 3 left navigation probe: PASS")
end

local ok, err = pcall(main)
if not ok then
    log("WHAT IF Phase 3 left navigation probe: FAIL - " .. tostring(err))
end
