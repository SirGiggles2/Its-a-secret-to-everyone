...
local OUT_PATH = OUT_DIR .. "bizhawk_phase3_cave_room78_probe.txt"
...
local TARGET_ROOM = 0x0078
local TARGET_EXIT_X = 80
local TARGET_EXIT_Y = 141
...
local function main()
    ...
    move_to_room_78()
    if not seek_room78_cave_entry() then
        error("never entered cave from room 0078")
    end
    ...
    if enter_started == nil then
        error("room78 enter transition never started")
    end
    ...
    for _ = 1, 240 do
        ...
        if enter_prev ~= nil then
            ...
            if dy ~= 0 then
                ...
                if dy ~= 1 then
                    error(string.format("room78 enter step expected 1px, got %d", dy))
                end
            end
        end
        enter_prev = y
    ...
    if not enter_finished then
        error("room78 enter transition never finished")
    end
    if enter_steps == 0 then
        error("room78 enter had zero walk samples")
    end

    local cave_settle_x = read_u16(LINK_X)
    log(string.format("room78 cave settle x=%d y=%d", cave_settle_x, read_u16(LINK_Y)))
    if cave_settle_x ~= CAVE_INTERIOR_X then
        error(string.format("expected room78 cave interior X=%d, got %d", CAVE_INTERIOR_X, cave_settle_x))
    end
    ...
    if exit_started == nil then
        error("room78 exit transition never started")
    end
    ...
    for _ = 1, 480 do
        ...
        if read_u16(ROOM_CONTEXT_MODE) ~= 1 then
            error("room78 context switched before exit transition finished")
        end
        ...
        if dy ~= 0 then
            ...
            if dy ~= 1 then
                error(string.format("room78 exit step expected 1px, got %d", dy))
            end
            local phase = frame_counter & 0x0003
            if exit_phase == nil then
                exit_phase = phase
            elseif phase ~= exit_phase then
                error(string.format("room78 exit cadence drifted: expected phase %d got %d", exit_phase, phase))
            end
        end
        prev_y = y
    end
    if exit_steps == 0 then
        error("room78 exit had zero walk samples")
    end
    ...
    if exited == nil then
        error("room78 never exited cave mode")
    end

    local end_room = read_u16(ROOM_ID)
    local end_x = read_u16(LINK_X)
    local end_y = read_u16(LINK_Y)
    log(string.format("room78 exit frame=%d room=%04X x=%d y=%d", exited, end_room, end_x, end_y))

    if end_room ~= TARGET_ROOM then
        error(string.format("expected return room 0078, got %04X", end_room))
    end
    ...
    log("WHAT IF Phase 3 cave room78 probe: PASS")
end

local ok, err = pcall(main)
if not ok then
    log("WHAT IF Phase 3 cave room78 probe: FAIL - " .. tostring(err))
end
