...
-- T34 Genesis trace: mirror of bizhawk_t34_movement_nes_capture.lua on the
-- Genesis core. Drives Link through the same scripted D-pad square walk in
-- room $77, reading NES-mirror addresses at $FF00xx. Pairs with
-- bizhawk_t34_movement_nes_capture.lua for byte-parity comparison.
--
-- Outputs:
--   builds/reports/t34_movement_gen_capture.txt
--   builds/reports/t34_movement_gen_capture.json
--   builds/reports/t34_movement_gen_capture.png
...
local OUT_TXT  = repo_path("builds\\reports\\t34_movement_gen_capture.txt")
local OUT_JSON = repo_path("builds\\reports\\t34_movement_gen_capture.json")
local OUT_PNG  = repo_path("builds\\reports\\t34_movement_gen_capture.png")
...
local function vram_u16(addr) return try_read("VRAM", addr, 2) or 0 end

-- ---------------------------------------------------------------------------
-- Input
-- ---------------------------------------------------------------------------
local function safe_set(pad)
    -- Accept both "Right" and "P1 Right" forms — set both so we work across cores
    local expanded = {}
...
local function read_exception_block()
    -- Only flag on real exception types: 2=bus error, 3=addr error.
    -- Uninit RAM and default ($FF0900 == 0) are safe.
...
local SAT_CHECKPOINT_TS = {0, 59, 60, 119, 120, 134, 135, 194, 195, 209,
                          210, 269, 270, 284, 285, 344, 345, 360}
local SAT_CP_SET = {}
for _, t in ipairs(SAT_CHECKPOINT_TS) do SAT_CP_SET[t] = true end
...
for frame = 1, MAX_FRAMES do
    ...
    if exc_count > 0 and CAPTURE.gen_exception_frame == nil then
        ...
        record(string.format("f%04d GEN EXCEPTION DETECTED — halting", frame))
        break
    end
    ...
    elseif flow_state == FLOW_T34_CAPTURE then
        ...
        local obj_x  = ram_u8(A_OBJ_X)
        local obj_xf = ram_u8(A_OBJ_XF)
        local obj_y  = ram_u8(A_OBJ_Y)
        local obj_yf = ram_u8(A_OBJ_YF)
        local obj_dir = ram_u8(A_OBJ_DIR)
        local held   = ram_u8(A_HELD)
        CAPTURE.trace[#CAPTURE.trace + 1] = {
            t = t,
        ...
        if SAT_CP_SET[t] then
            local s0 = vram_u16(VDP_SAT_BASE + 0)
            local s2 = vram_u16(VDP_SAT_BASE + 2)
            local s4 = vram_u16(VDP_SAT_BASE + 4)
            local s6 = vram_u16(VDP_SAT_BASE + 6)
            CAPTURE.sat_checkpoints[#CAPTURE.sat_checkpoints + 1] = {
                t = t, w0 = s0, w1 = s2, w2 = s4, w3 = s6,
            }
        end
    end
...
local function build_json()
    ...
    local sat_parts = {}
    for i, c in ipairs(CAPTURE.sat_checkpoints) do
        sat_parts[i] = string.format(
            '{"t":%d,"w0":%d,"w1":%d,"w2":%d,"w3":%d}',
            c.t, c.w0, c.w1, c.w2, c.w3)
    end
    ...
    return table.concat({
        ...
        '"sat_checkpoints":[' .. table.concat(sat_parts, ",") .. "],",
        '"trace":{',
        '"t":'      .. json_num_array(t_a)  .. ",",
        '"obj_x":'  .. json_num_array(x_a)  .. ",",
        '"obj_xf":' .. json_num_array(xf_a) .. ",",
...
local verdict = ok_capture and "T34_GEN_CAPTURE: OK" or "T34_GEN_CAPTURE: FAIL"
record(verdict)
