#!/usr/bin/env python3
"""T37 sword-pickup byte-parity comparator.
...
Pickup semantics:
  ...
  sets slot 0 = 1 (wood sword).  T37 PASS requires:
...
Gates (9):
  T37_NES_CAPTURE_OK
  T37_GEN_CAPTURE_OK
  T37_NO_GEN_EXCEPTION
  T37_BASELINE_PARITY          t=0..59 (obj_x, obj_y, mode, room) match
  T37_CAVE_ENTER_TRIGGERED     NES & Gen both leave mode $05 near t=213
  T37_SWORD_PICKUP_NES         NES inv_sword 0->>=1 fires between 491..701
  T37_SWORD_PICKUP_GEN         Gen inv_sword 0->>=1 fires between 491..701
  T37_PICKUP_PHASE_MATCH       NES & Gen pickup frames within +/-PHASE_TOLERANCE
  T37_ROUND_TRIP_READY         final (room, mode, obj_x, obj_y) match
"""
...
NES_JSON = REPORTS / "t37_sword_nes_capture.json"
GEN_JSON = REPORTS / "t37_sword_gen_capture.json"
OUT_TXT  = REPORTS / "bizhawk_t37_sword_parity_report.txt"
OUT_JSON = REPORTS / "bizhawk_t37_sword_parity_report.json"
...
def main() -> int:
    ...
    emit("=" * 65)
    emit("T37 SWORD-PICKUP PARITY REPORT (NES vs Genesis, cave $77)")
    emit("=" * 65)
    ...
    gate("T37_NES_CAPTURE_OK", nes_ok,
    ...
    gate("T37_GEN_CAPTURE_OK", gen_ok,
         f"len={len(gen['trace']['t']) if gen else 0}" if gen else "MISSING")

    exc = gen.get("gen_exception_frame") if gen else None
    gate("T37_NO_GEN_EXCEPTION", exc is None,
         "no exception" if exc is None else f"exception at frame {exc}")

    if not (nes_ok and gen_ok):
        for n in ("T37_BASELINE_PARITY", "T37_CAVE_ENTER_TRIGGERED",
                  "T37_SWORD_PICKUP_NES", "T37_SWORD_PICKUP_GEN",
                  "T37_PICKUP_PHASE_MATCH", "T37_ROUND_TRIP_READY"):
    ...
    for k in ("obj_x", "obj_y", "mode", "room"):
        ...
        if d:
            ...
            dets.append(f"{k}:OK")
    gate("T37_BASELINE_PARITY", pre_ok, "; ".join(dets))
    ...
    if nes_inv:
        for t in range(PICKUP_WINDOW_LO, min(PICKUP_WINDOW_HI, len(nes_inv))):
            if nes_inv[t] >= 1 and nes_inv[t - 1] == 0:
                nes_pickup = t
                break
    gate("T37_SWORD_PICKUP_NES", nes_pickup is not None,
    ...
    if gen_inv:
        for t in range(PICKUP_WINDOW_LO, min(PICKUP_WINDOW_HI, len(gen_inv))):
            if gen_inv[t] >= 1 and gen_inv[t - 1] == 0:
                gen_pickup = t
                break
    gate("T37_SWORD_PICKUP_GEN", gen_pickup is not None,
    ...
    if nes_pickup is not None and gen_pickup is not None:
        ...
        gate("T37_PICKUP_PHASE_MATCH", phase_ok,
             f"delta={gen_pickup - nes_pickup:+d}f (NES t={nes_pickup}, Gen t={gen_pickup})")
    else:
        gate("T37_PICKUP_PHASE_MATCH", False,
    ...
    gate("T37_ROUND_TRIP_READY", final_ok,
...
def write(results, lines, all_pass):
    verdict = "T37_PARITY: ALL PASS" if all_pass else "T37_PARITY: FAIL"
