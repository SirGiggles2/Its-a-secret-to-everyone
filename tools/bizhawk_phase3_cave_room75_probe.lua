...
local OUT_PATH = OUT_DIR .. "bizhawk_phase3_cave_room75_probe.txt"
...
local function assert_enter_auto_walk()
    ...
    log(string.format("room75 cave settle x=%d y=%d", x, read_u16(LINK_Y)))
end
...
local function assert_exit_auto_walk()
    ...
    log(string.format("room75 exit frame=1 room=%04X x=%d y=%d", read_u16(ROOM_ID), x, y))
end
...
local function run_probe()
    ...
    move_to_room_75()
    local entered = seek_room75_cave_entry()
    if not entered then
    ...
    log("WHAT IF Phase 3 cave room75 probe: PASS")
end
