...
-- bizhawk_t35_scroll_gen_capture.lua for byte-parity comparison by
-- tools/compare_t35_scroll_parity.py.
--
-- Outputs:
--   builds/reports/t35_scroll_nes_capture.txt
--   builds/reports/t35_scroll_nes_capture.json
--   builds/reports/t35_scroll_nes_capture.png
...
local OUT_TXT  = repo_path("builds\\reports\\t35_scroll_nes_capture.txt")
local OUT_JSON = repo_path("builds\\reports\\t35_scroll_nes_capture.json")
local OUT_PNG  = repo_path("builds\\reports\\t35_scroll_nes_capture.png")
...
local FLOW_T35_STABILIZE     = "T35_STABILIZE"
local FLOW_T35_CAPTURE       = "T35_CAPTURE"
...
record("=================================================================")
record("T35 NES capture: room $77 -> $76 left-scroll reference trace")
record("=================================================================")
...
for frame = 1, MAX_FRAMES do
    ...
    elseif flow_state == FLOW_FS1_START_GAME then
        if mode ~= 0x01 then
            set_flow(FLOW_T35_STABILIZE, frame, "left Mode1 (gameplay starting)")
        else
    ...
    elseif flow_state == FLOW_T35_STABILIZE then
        if mode == 0x05 and room_id == TARGET_ROOM_ID then
            ...
            if CAPTURE.stable_count >= LINK_STABLE_FRAMES then
                ...
                set_flow(FLOW_T35_CAPTURE, frame, "T35 capture window open")
    ...
    elseif flow_state == FLOW_T35_CAPTURE then
...
local verdict = (trace_len == SCENARIO.SCENARIO_LENGTH) and "T35_NES_CAPTURE: OK" or "T35_NES_CAPTURE: FAIL"
record(verdict)
write_file(OUT_TXT, table.concat(lines, "\n") .. "\n")
