...
def _emit_punct_chr(out_path: Path) -> None:
    ...
    if gen_copy.exists():
        ...
        return
    # Fallback: use the checked-in generated asset first. The old
    # tools/intro_demo source was removed after the generated data moved
    # under data/intro.
    src_path = REPO / "data" / "intro" / "intro_punct_chr.c"
    legacy_src_path = REPO / "tools" / "intro_demo" / "intro_punct_chr.c"
    if not src_path.exists() and legacy_src_path.exists():
        src_path = legacy_src_path
    if not src_path.exists():
        raise FileNotFoundError(
            f"intro_punct_chr.c not found at {src_path}, {legacy_src_path}, "
            f"or {gen_copy}. Restore data/intro/intro_punct_chr.c from git."
        )
    ...
    text = text.replace(
        ...
        "/* Auto-generated by tools/extract_intro_assets.py - do not edit.\n"
        " * Source: data/intro/intro_punct_chr.c (hand-written).",
    )
