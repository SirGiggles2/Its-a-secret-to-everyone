...
def _build_story_tilemap(ref_dir: Path) -> tuple[list[int], int]:
    """Compose the static story-scroll nametable strip for NT1.

    Source: StoryTileAttrTransferBuf.dat — 30 nametable rows at $2000-$23A0
    (32 tile bytes each), followed by two 32-byte attribute records at
    $23C0 and $23E0 (full NT1 attribute table), plus trailing NT2 attribute
    patches we ignore (the native rewrite targets a single Genesis plane).

    Each NES attribute byte covers a 4x4 tile quad; bits 1:0 = top-left
    2x2 palette, 3:2 = top-right, 5:4 = bottom-left, 7:6 = bottom-right.

    Genesis output cell: (palette << 13) | tile_idx.
    """
