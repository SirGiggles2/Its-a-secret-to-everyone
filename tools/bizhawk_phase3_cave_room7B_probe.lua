...
local OUT_PATH = OUT_DIR .. "bizhawk_phase3_cave_room7B_probe.txt"
...
local function assert_enter_auto_walk()
    ...
    log(string.format("room7B cave settle x=%d y=%d", x, read_u16(LINK_Y)))
end
...
local function assert_exit_auto_walk()
    ...
    log(string.format("room7B exit frame=1 room=%04X x=%d y=%d", read_u16(ROOM_ID), x, y))
end
...
local function run_probe()
    ...
    move_to_room_7B()
    local entered = seek_room7b_cave_entry()
    if not entered then
    ...
    log("WHAT IF Phase 3 cave room7B probe: PASS")
end
