...
-- T35 Genesis trace: mirror of bizhawk_T37_cave_nes_capture.lua on the
...
--
-- Outputs:
--   builds/reports/t37_sword_gen_capture.txt
--   builds/reports/t37_sword_gen_capture.json
--   builds/reports/t37_sword_gen_capture.png
...
local SCENARIO = dofile(repo_path("tools\\t37_input_scenario.lua"))
...
local OUT_TXT  = repo_path("builds\\reports\\t37_sword_gen_capture.txt")
local OUT_JSON = repo_path("builds\\reports\\t37_sword_gen_capture.json")
local OUT_PNG  = repo_path("builds\\reports\\t37_sword_gen_capture.png")

-- T39 render-classification shot manifest. Helpers (t39_dump_nt /
-- t39_dump_vdp) live further down, after ram_u8 is declared.
-- T37 visual waypoints. Times match the sword-pickup scenario:
--   t=40   overworld pre-cave   t=400  cave interior pre-sword
--   t=600  cave w/ sword held   t=900  post-exit back on overworld
local T39_SHOTS = {
    { t = 40,  png = repo_path("builds\\reports\\t37_gen_pre.png"),
               nt  = repo_path("builds\\reports\\t37_nt_gen_pre.hex"),
               vdp = repo_path("builds\\reports\\t37_vdp_gen_pre.hex") },
    { t = 400, png = repo_path("builds\\reports\\t37_gen_incave.png"),
               nt  = repo_path("builds\\reports\\t37_nt_gen_incave.hex"),
               vdp = repo_path("builds\\reports\\t37_vdp_gen_incave.hex") },
    { t = 600, png = repo_path("builds\\reports\\t37_gen_pickup.png"),
               nt  = repo_path("builds\\reports\\t37_nt_gen_pickup.hex"),
               vdp = repo_path("builds\\reports\\t37_vdp_gen_pickup.hex") },
    { t = 900, png = repo_path("builds\\reports\\t37_gen_post.png"),
               nt  = repo_path("builds\\reports\\t37_nt_gen_post.hex"),
               vdp = repo_path("builds\\reports\\t37_vdp_gen_post.hex") },
}
...
-- T37: inventory array. Slot 0 = sword level (0=none, 1=wood, ...).
...
local FLOW_T37_STABILIZE   = "T37_STABILIZE"
local FLOW_T37_CAPTURE     = "T37_CAPTURE"
...
-- ---------------------------------------------------------------------------
local BP_LOG = repo_path("builds\\reports\\T37_cave_statewrite.txt")
local bp_lines = {}
local bp_count = 0
local BP_MAX = 4000
...
for frame = 1, MAX_FRAMES do
    ...
    if flow_state == FLOW_REPLAY then
        ...
        -- see Mode5/room77 + Link stable. At that point switch to T37_STABILIZE
        ...
        if mode == 0x05 and room_id == TARGET_ROOM_ID then
            ...
            set_flow(FLOW_T37_STABILIZE, frame, "replay hit gameplay")
        end
    ...
    elseif flow_state == FLOW_FS1_START_GAME then
        if mode ~= 0x01 then
            set_flow(FLOW_T37_STABILIZE, frame, "left Mode1 (gameplay starting)")
        else
            local target = 0x00
    ...
    elseif flow_state == FLOW_T37_STABILIZE then
        if mode == 0x05 and room_id == TARGET_ROOM_ID then
            if not CAPTURE.reached_mode5 then
                CAPTURE.reached_mode5 = true
                CAPTURE.reached_mode5_frame = frame
            ...
            -- failure of T37_CAVE_INTERIOR_MATCH / T37_ROUND_TRIP_READY.
            ...
            if CAPTURE.stable_count >= LINK_STABLE_FRAMES
               and (frame_ctr % 4) == 0 then
                ...
                set_flow(FLOW_T37_CAPTURE, frame, "T35 capture window open")
    ...
    elseif flow_state == FLOW_T37_CAPTURE then
        ...
        CAPTURE.trace[#CAPTURE.trace + 1] = {
            ...
            sram_68fe = ram_u8(A_SRAM_68FE),
            sram_lvl = ram_u8(A_SRAM_68FE + ram_u8(A_LVL_BLOCK)),
            inv_sword = ram_u8(A_INV_SWORD),  -- T37: sword inventory slot
        }
    end

    local pad
    if flow_state == FLOW_T37_CAPTURE then
...
local verdict = ok_capture and "T37_GEN_CAPTURE: OK" or "T37_GEN_CAPTURE: FAIL"
record(verdict)
