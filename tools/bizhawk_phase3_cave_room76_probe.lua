...
local OUT_PATH = OUT_DIR .. "bizhawk_phase3_cave_room76_probe.txt"
...
local function find_and_enter_room76_cave()
    for _ = 1, 120 do
        ...
        if read_u16(ROOM_ID) ~= TARGET_ROOM then
            error(string.format("left room 0076 while searching cave door, now in %04X", read_u16(ROOM_ID)))
        end
...
local function main()
    ...
    move_to_room_76()
    local entered = find_and_enter_room76_cave()
    if not entered then
        error("never entered cave from room 0076")
    end
    local entered_frame = 1
    log(string.format("entered cave frame=%d room=%04X x=%d y=%d", entered_frame, read_u16(ROOM_ID), read_u16(LINK_X), read_u16(LINK_Y)))

    local enter_started = wait_for(function()
        return read_u16(CAVE_TRANSITION_MODE) == 1
    end, 120, {})
    if enter_started == nil then
        error("room 0076 enter transition never started")
    end
    ...
    for _ = 1, 240 do
        ...
        if prev_y ~= nil then
            ...
            if dy ~= 0 then
                ...
                if dy ~= 1 then
                    error(string.format("room 0076 enter step expected 1px, got %d", dy))
                end
            end
        end
        prev_y = y
    ...
    if not enter_finished then
        error("room 0076 enter transition never finished")
    end
    if enter_steps == 0 then
        error("room 0076 enter had zero walk samples")
    end

    local cave_settle_x = read_u16(LINK_X)
    local cave_settle_y = read_u16(LINK_Y)
    log(string.format("room76 cave settle x=%d y=%d", cave_settle_x, cave_settle_y))
    if cave_settle_x ~= CAVE_INTERIOR_X then
        error(string.format("expected room76 cave interior X=%d, got %d", CAVE_INTERIOR_X, cave_settle_x))
    end
    ...
    if exit_started == nil then
        error("room 0076 exit transition never started")
    end

    local exit_phase = nil
    local exit_steps = 0
    prev_y = read_u16(LINK_Y)
    for _ = 1, 480 do
        frame_with_input({})
        local mode = read_u16(CAVE_TRANSITION_MODE)
        if mode ~= 2 then
        ...
        if read_u16(ROOM_CONTEXT_MODE) ~= 1 then
            error("room76 context switched before exit transition finished")
        end
        ...
        if dy ~= 0 then
            ...
            if dy ~= 1 then
                error(string.format("room76 exit step expected 1px, got %d", dy))
            end
            local phase = frame_counter & 0x0003
            if exit_phase == nil then
                exit_phase = phase
            elseif phase ~= exit_phase then
                error(string.format("room76 exit cadence drifted: expected phase %d got %d", exit_phase, phase))
            end
        end
        prev_y = y
    end
    if exit_steps == 0 then
        error("room76 exit had zero walk samples")
    end
    ...
    if exited == nil then
        error("room 0076 never exited cave mode")
    end

    local end_room = read_u16(ROOM_ID)
    local end_x = read_u16(LINK_X)
    local end_y = read_u16(LINK_Y)
    log(string.format("room76 exit frame=%d room=%04X x=%d y=%d", exited, end_room, end_x, end_y))

    if end_room ~= TARGET_ROOM then
        error(string.format("expected return room 0076, got %04X", end_room))
    end
    ...
    log("WHAT IF Phase 3 cave room76 probe: PASS")
end

local ok, err = pcall(main)
if not ok then
    log("WHAT IF Phase 3 cave room76 probe: FAIL - " .. tostring(err))
end
