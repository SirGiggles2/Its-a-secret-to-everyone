...
- [ ] **Rule D1 (Drain-First / NES-Disasm-Second).** Drained C in `src/game/<subsystem>/*_runtime.c` is the **PRIMARY implementation evidence**. NES disassembly (`reference/aldonunez/*.asm`, `src/zelda_translated/*.asm`) is the **SECONDARY verification + final authority** — it wins ties when drain is wrong. Per debate 005 (2026-05-02). Every Task gets a 4-line drain-aware header before its body:
  ...
  ```
  - **ADOPT** = drain logic correct + already wired or trivially wirable; task body = integration only.
  - **EXTEND** = drain logic correct, needs Genesis-native swap (replace `_ppu_*`/`_apu_*` shims with `vdp_*`/`xgm_*` calls); behavior preserved.
...
- [ ] **NO compatibility aliases.** `build.bat` MUST emit only `Title.md` / `Title.lst` / `Title.o` / `Title.elf`. The legacy `whatif.*` alias is removed permanently per user hard rule (debate 004, 2026-05-02). Every probe and launcher must reference `Title.*` directly. Enforced by `tools/gates/check_no_whatif.py` (CI fail on any `whatif` reference outside `debates/` and `docs/archive/`).
- [ ] Build once and confirm only `Title.*` artifacts in `builds/`.

### Task 0.3: Rename Probe Inputs

- [ ] Update `tools/probe_addresses.lua` to read `builds\Title.lst`. No `whatif.lst` fallback (legacy alias dead per Task 0.2).
...
- Substrate (main worktree): `src/state/cave_state.h` (typed view shipped this session)
- New (RoomRom worktree): `RoomRom/src/roomrom_cave_dispatch.c`, gameplay-loop wire-up
- Generated data (Task 3.1): `RoomRom/data/caves.json` + C blobs from extractor
- Probes: `RoomRom/probe_nes_cave_manifest.lua`, `RoomRom/probe_roomrom_caves.lua`

### Task 3.1 — Build Cave Source Data Manifest

- **NES source**: `reference/aldonunez/Z_01.asm:CaveSpec_*`, `:CavePalettes`, `:CaveItemPrices`, `:CaveTextIds`, `:CaveTransferAddresses`
- **Drained C**: NONE (data extraction, not behavior)
- **Coverage**: NONE
- **Stance**: GREENFIELD (legal — pure data pipeline; no behavior to drain)

Steps:
- [ ] Extractor `RoomRom/tools/gen_cave_data.py`: parse NES disasm tables (CaveSpec / CavePalettes / CaveItemPrices / CaveTextIds / CaveTransferAddresses) + Redux modifications.
- [ ] Emit `RoomRom/data/caves.json` (canonical) + C blob `.c/.h` for build.
- [ ] Manifest hash; verify against NES ROM byte content.
- [ ] Per-cave entries: room id, cave type, NPC type, text id, item/shop inventory, prices, item grant rules, palette ref, tilemap template ref.

### Task 3.2 — Cave Gamemode Dispatch

- **NES source**: `reference/aldonunez/Z_01.asm:GameMode_Cave`, `:CaveInit`, `:CaveTick`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_init_cave`, `cavert_update_cave_person`
- **Coverage**: PARTIAL — init + person update drained. MISSING: Genesis gamemode-table wire-up; RoomRom dispatch entry.
- **Stance**: ADOPT + EXTEND (logic adopted as-is; wire into Genesis gamemode table)

Steps:
- [ ] Read cavert_init_cave + cavert_update_cave_person end-to-end. Gate 1 diff vs NES asm — log to `tools/audit/drain_findings/cave_3_2.md`.
- [ ] Add scene enum value for cave; add cave-related fields to typed `CaveState` if missing.
- [ ] RoomRom dispatch: register cave gamemode handler that calls cavert_init_cave on entry, cavert_update_cave_person per frame.
- [ ] Build RoomRom; smoke-test boot → enter cave → tick.

### Task 3.3 — Cave Entrance Detection

- **NES source**: `reference/aldonunez/Z_01.asm:CheckCaveEntry`, `Z_02.asm:OW_TileCollision`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_init_cave_continue` (entry transition), entry-point detection drained in `src/game/world/` overworld code.
- **Coverage**: PARTIAL — entry transition drained. MISSING: cave entrance tile detection in overworld collision (may live in `src/game/world/` or RoomRom OW renderer).
- **Stance**: EXTEND (run drain audit first to find entrance-detection function; if absent → REPLACE sub-task with evidence)

Steps:
- [ ] Run `python tools/audit/drain_coverage.py --phase 3` and grep `src/game/world/` for cave-entry detection.
- [ ] If drained: ADOPT + wire into RoomRom OW collision path.
- [ ] If absent: file evidence, REPLACE with new RoomRom-side detector at known cave-tile coords.
- [ ] Store return room + coordinates in CaveState; trigger cavert_init_cave; gate normal edge transition.

### Task 3.4 — Render Cave Interior

- **NES source**: `reference/aldonunez/Z_01.asm:DrawCave`, `:DrawCaveContents`, `:Cave_BlitWalls`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_draw_cave_person`, `cavert_draw_cave_items`; `src/game/cave/uw_person_runtime.c` (UW NPC draw)
- **Coverage**: PARTIAL — drain covers wall/floor blit, NPC + item slot positions, palette load. MISSING: Genesis VDP plane writes (drain still calls `_ppu_write_*` shims), CRAM lane assignment for cave PAL2/PAL3, scroll register init for cave's fixed-camera mode.
- **Stance**: EXTEND

Steps:
- [ ] Read cavert_draw_cave_person + cavert_draw_cave_items end-to-end. Gate 1 diff vs `DrawCave` / `DrawCaveContents`.
- [ ] Replace `_ppu_write_*` shim calls with `vdp_plane_a_write` / `vdp_dma_chr` (Genesis-native swap; behavior preserved per Rule WT-1 substrate paths).
- [ ] Wire cave_draw_* into Phase 3 cave gamemode dispatch.
- [ ] Smoke: boot → enter cave 1 → screenshot → compare against `build/generated/nes_reference/8f72dc2e/cave_*` capture.

### Task 3.5 — Cave Text Box

- **NES source**: `reference/aldonunez/Z_01.asm:CaveText_*`, `:TextBox_Render`, `:TextSpeed_Tick`; `Z_05.asm:Text_RenderChar`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_update_person_state_textbox`, `cavert_format_decimal_byte`, `cavert_write_prices_transfer_buf`
- **Coverage**: PARTIAL — textbox state machine + decimal formatter drained. MISSING: actual char rendering (likely in `src/game/hud/` or text_runtime.c if exists), text speed option hook (Phase 9 options runtime).
- **Stance**: EXTEND (cross-subsystem wire-up)

Steps:
- [ ] Drain audit: grep `src/game/hud/` and `src/game/world/` for text rendering. If found, ADOPT.
- [ ] Wire cavert_update_person_state_textbox into cave gamemode tick.
- [ ] Probe "IT'S DANGEROUS TO GO ALONE" + shop text + generic old man hint.

### Task 3.6 — Item Grant Caves

- **NES source**: `reference/aldonunez/Z_01.asm:CaveItemGrant`, `:GiveItem`; `Z_07.asm:Inventory_Mutate`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_try_take_item`, `cavert_try_take_room_item`
- **Coverage**: PARTIAL — item-take drained. MISSING: inventory mutation + sound trigger may live in `src/game/items/`; save flag write may live in `src/game/world/progress_runtime.c`.
- **Stance**: ADOPT + EXTEND (wire-up only; cross-subsystem)

Steps:
- [ ] Drain audit: confirm cavert_try_take_item calls inventory mutation function (drain or RAM write); confirm save flag write.
- [ ] Verify sword cave, heart container caves, letter cave, white/magical sword caves all dispatched correctly via cave_id from caves.json.
- [ ] Probe one item-grant cave end-to-end.

### Task 3.7 — Shop Caves

- **NES source**: `reference/aldonunez/Z_01.asm:Shop_Tick`, `:Shop_Purchase`, `:Shop_Render3Items`, `:DeductRupees`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_update_talk_shop_or_door_charge`, `cavert_update_transfer_prices`, `cavert_clear_prices_cave_flag`, `cavert_write_prices_to_dynamic_transfer_buf`, `cavert_update_hint_or_money_game` (also covers door-repair charge)
- **Coverage**: FULL on logic. MISSING: HUD redraw hook for rupee deduct (likely in HUD subsystem).
- **Stance**: ADOPT (logic) + EXTEND (HUD hook)

Steps:
- [ ] Drain audit confirms cavert_update_talk_shop_or_door_charge handles all 3 shop types + door-repair.
- [ ] Verify rupee deduction calls inventory mutation that triggers HUD redraw.
- [ ] Probe each shop type (3-item, secret moblin, door-repair).

### Task 3.8 — Gambling / Money-Making Game

- **NES source**: `reference/aldonunez/Z_01.asm:MoneyGame_Tick`, `:MoneyGame_Reward`, `:Z_Rand`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_update_hint_or_money_game` (handles both hint caves and money-making game per name)
- **Coverage**: FULL on logic. Verify against NES asm — name suggests dispatcher splits hint vs money-game branches.
- **Stance**: ADOPT (verification only)

Steps:
- [ ] Gate 1 diff: cavert_update_hint_or_money_game vs MoneyGame_Tick — log MATCH/DIFF to drain_findings.
- [ ] Verify deterministic seeded run produces NES-identical reward sequence.

### Task 3.9 — Cave Exit + Palette Restore

- **NES source**: `reference/aldonunez/Z_01.asm:ExitCave`, `:RestoreOverworldPalette`
- **Drained C**: `src/game/cave/cave_runtime.c:cavert_update_person_state_delay_then_hide` (covers cave→OW transition); palette restore likely in `src/game/world/`
- **Coverage**: PARTIAL — exit transition drained. MISSING: palette restore wire-up (cross-subsystem).
- **Stance**: EXTEND

Steps:
- [ ] Drain audit: locate palette restore function in `src/game/world/` or `src/game/cave/`.
- [ ] Wire exit transition: detect Link walks out → cavert_update_person_state_delay_then_hide → palette restore → return-room reload.
- [ ] Probe re-enter cave shows item gone (collected-flag preserved).
...
- **Drained C**: N/A
- **Coverage**: NONE
- **Stance**: GREENFIELD (verification orchestration, not behavior)

Gates:
- [ ] **Gate 1 (per-function diff):** `tools/audit/drain_findings/cave_*.md` complete for every cavert_* function exercised by Phase 3 tasks.
- [ ] **Gate 2 (per-RAM-cell trace):** parity oracle scenario `cave_state_trace_60f` snapshots `$00EB` (RoomId), `$00AC` (CavePersonState), `$0413` (CaveFlags), `$0421-$0438` (cave shop ware state), `$046C-$0471` (money game), `$066D` (LinkRupees) across 60 frames — diff Genesis vs NES.
- [ ] **Gate 3 (per-scenario oracle):** scenarios `cave_enter_take_any`, `cave_buy_candle_three_item`, `cave_door_repair_pay`, `cave_money_game_seeded`, `cave_secret_moblin_30r` — all produce screenshot + RAM diff matching NES.
- [ ] `tools/audit/drain_coverage.py` exit 0 for phase 3 (all task headers valid; zero phantoms; orphans for cave subsystem either cited or documented as not-Phase-3-scope).
- [ ] Save report `builds/reports/roomrom_caves_matrix.md`.
- [ ] Commit as `roomrom: implement overworld caves (drain-aware)`.
...
- **9.2 SRAM ABI / save format** → `main` (substrate): `src/state/save_state.h`, save serialization in `src/abi/`.
- **9.3 Options menu UI** → `main` (frontend): `src/frontend/options/options_menu.c` (renders during file-select context).
- **9.4 HUD options readout** → `RoomRom` (gameplay): consumers of options state during gameplay.
- **9.5 HUD rendering** → `RoomRom`: hearts/rupees/keys/bombs counters, item-get overlay.
- **9.6 Pause-menu invocation** → `RoomRom`: triggered from gameplay loop.
- **9.7 Save / death / continue** → SPLIT: save serialization in `main` (substrate); death/continue gameplay in `RoomRom`; file-select return boundary in `main` (frontend).
...
**Worktree:** `main` owns the audio driver / driver-ABI / generated audio (`src/audio_driver.asm`, `src/sgdk_adapter/audio_adapter.c`, `data/audio/`). RoomRom owns gameplay event callsites only (writes `SongRequest` per the audio ABI contract). Shared audio-ABI edits trigger `tools/gates/check_substrate_dual_rom.py` (defense-in-depth). Migration trigger criteria for switching the custom driver to XGM2 live in `docs/audio_migration_trigger.md` (debate 003 Rule SGDK-4).
...
**Worktree:** `main` (Title.md frontend gap-fill; edits only `src/frontend/*`).

**Ordering:** PARALLEL-ELIGIBLE with Phases 2-8 because frontend touches only `src/frontend/*` (no overlap with RoomRom gameplay paths). GATED at Phase 12 — both ROMs must pass `tools/gates/check_substrate_dual_rom.py` + `tools/audit/merge_readiness.py` GREEN before Phase 12 promotion flips Title.md's main-loop dispatch from transpiled fallback to promoted gameplay.
...
- **T0.3 cave BG via OW renderer SHIPPED** (`RoomRom/src/main.c` boot auto-entry + C+START toggle) — `roomrom_ow_room_render_fill_plane_a(cid)` + `_publish_play_area_tiles()` paint plane A on SCENE_CAVE entry; restore via `roomrom_ow_room_render_fill_plane_a(s_room_id)` on exit. Follow-up: cave-id $6A renders OW room $6A's water/rock layout instead of real NES cave-interior (black bg + brown floor + cave-person on platform); separate Z_01.asm cave-data port pending (task #44).
- **#42 cave palette swap SHIPPED** (`src/game/world/render/cave_palette.{c,h}`) — NES `Z_06.asm:714 CaveBgPaletteRowsTransferBuf` $3F08 8-byte palette ($0F $30 $00 $12 / $0F $07 $0F $17) loaded into Gen PAL0 slots 8..15 via `render_cram_subrange_upload`. Restore via `roomrom_ow_room_render_load_palette(s_room_id)` on cave exit. Probe shot1 confirms blue/gray cave subpal 2 visible.
- **T5.0 audio probe sweep SHIPPED** (`docs/audit/t5_0_audio_probe_sweep.md`) — verdict: vblank budget GREEN (300/300 frames fit), music event log PARTIAL (gamemode stuck at $0C, dispatcher never fires), SFX sweep PARTIAL (driver BSS active but probe noisy), low-health PARTIAL (SRAM uninit), FS routing RED (gamemode never reaches $01). Spawned three v5b prereqs: T5.0.1 investigate $0C park, T5.0.2 find dmc_last_idx address, T5.0.3 decide m_song mirror to nes_ram[$88]. All T5.1+ audio wiring blocked behind T5.0.1.
