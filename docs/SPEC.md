...
| T38 | Enemy AI | Enemies spawn, move, and respond to hits | ⚠ Partial (2026-04-18) — edge-spawn rooms (bit 3 of `RoomAttrsOW_F`, ~20 OW rooms including $73/$74/$5B/$68) had all enemies stuck at Y=$DD with tile-check blocking every direction. Fix chain P42–P47 (`75602a8e`, `f9e9e9ed`): **P42** forces `AssignObjSpawnPositions` to run normal-spawn in OW (was short-circuiting for bit-3 rooms); **P43** adds UninitMonsterFromEdge fresh-room-entry gate (Codex-proposed — preserve SpawnPosList positions when `$004B=0`, `$000F=$FF`, residual X/Y non-zero); **P44** adds Gen-native `SpawnPosListPtrsGen` 32-bit pointer table (was indirecting NES $86xx → Gen RAM $FF86xx = garbage); **P46** reseeds `CurEdgeSpawnCell ($0525)` to $48 (top-edge mid) on entry to edge-spawn rooms; **P47** pre-walk + post-walk $0525 validation — if FindNextEdgeSpawnCell walk terminates on bottom edge ($E0 hi-nibble) due to the corner boundary bug at $4F/$E0, snap to $48 and retry. Result at room $73 after 360 settle frames: Y=$DD count **0/12** (was 4/12 pre-fix); enemies in playfield 9/12, remaining 3 at Y=$3D top-edge walkable-down. |
...
**As of 2026-04-18** — edge-spawn room enemy bug (T38 partial) and HUD glyph rendering (T39 polish) fixed in patch chain P42–P47 (commits `75602a8e`, `f9e9e9ed`). All edge-spawn-flag OW rooms (~20 rooms including $73/$74/$5B/$68/$71/$78) now have enemies entering the playfield from the top edge instead of stuck at Y=$DD. `_transfer_tilebuf_fast` HUD-row guard now cave-mode-only, so overworld HUD text renders legitimate "B A"/"LIFE" glyphs instead of garbage. Zelda37.79+ is the first build with this chain green.
