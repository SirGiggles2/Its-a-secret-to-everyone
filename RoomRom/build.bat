...
echo [3] Regenerating uw_dark_rooms via tools/uw_dark_rooms_gen.py (Task 5.8)...
python "%REPO%\tools\uw_dark_rooms_gen.py"
if errorlevel 1 ( echo FAIL: uw_dark_rooms_gen.py & exit /b 1 )

echo [3] Compiling uw_dark_rooms.c (Task 5.8: dark-room manifest)...
"%GCC%" %CFLAGS% %INCS% -c "%PROJ%\data\uw_dark_rooms.c" -o "%OUT%\uw_dark_rooms.o"
if errorlevel 1 ( echo FAIL: uw_dark_rooms.c & exit /b 1 )

echo [3] Compiling uw_dark_meta.c (Task 5.8: dark-room accessor + lit-state)...
"%GCC%" %CFLAGS% %INCS% -c "%PROJ%\src\uw_dark_meta.c" -o "%OUT%\uw_dark_meta.o"
if errorlevel 1 ( echo FAIL: uw_dark_meta.c & exit /b 1 )

echo [3] Regenerating uw_item_rooms via tools/uw_item_rooms_gen.py (Task 5.9)...
python "%REPO%\tools\uw_item_rooms_gen.py"
if errorlevel 1 ( echo FAIL: uw_item_rooms_gen.py & exit /b 1 )

echo [3] Compiling uw_item_rooms.c (Task 5.9: item-room manifest)...
"%GCC%" %CFLAGS% %INCS% -c "%PROJ%\data\uw_item_rooms.c" -o "%OUT%\uw_item_rooms.o"
if errorlevel 1 ( echo FAIL: uw_item_rooms.c & exit /b 1 )

echo [3] Compiling uw_item_room_meta.c (Task 5.9: pickup wrapper)...
"%GCC%" %CFLAGS% %INCS% -c "%PROJ%\src\uw_item_room_meta.c" -o "%OUT%\uw_item_room_meta.o"
if errorlevel 1 ( echo FAIL: uw_item_room_meta.c & exit /b 1 )

echo [3] Compiling roomrom_candle_fire.c (Task 5.8.1: candle fire slot 8)...
"%GCC%" %CFLAGS% %INCS% -c "%PROJ%\src\roomrom_candle_fire.c" -o "%OUT%\roomrom_candle_fire.o"
if errorlevel 1 ( echo FAIL: roomrom_candle_fire.c & exit /b 1 )
