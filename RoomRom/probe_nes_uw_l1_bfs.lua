-- probe_nes_uw_l1_bfs.lua
-- Boot NES Zelda, warp into Level 1, BFS-walk every reachable dungeon room
-- via D-pad, capture NT/AT/PALRAM each. Re-warp + replay direction path
-- to reach each known room without long backtrack chains.
...
local OUT_JSON = (os.getenv and os.getenv("CODEX_UW_OUT_JSON")) or
                 "C:\\Users\\Jake Diggity\\Documents\\GitHub\\FINAL TRY\\RoomRom\\out\\nes_uw_l1_bfs.json"
local TARGET_LEVEL = tonumber((os.getenv and os.getenv("CODEX_UW_LEVEL")) or "1") or 1
    ...
    if ok and domains then
        for _, d in ipairs(domains) do
            ...
            if lower:find("pal") or lower:find("palette") then
                ...
                if size_ok and size and size <= 64 then PAL_DOMAIN = name; break end
            end
        end
    end
end
...
local function boot_to_overworld()
    ...
    for frame = 1, 20000 do
        local mode = u8(GAME_MODE); local slot = u8(CUR_SAVE_SLOT)
...
local function in_dungeon_play(level)
    return u8(GAME_MODE) == 0x05 and u8(GAME_SUB) == 0 and u8(CUR_LEVEL) == level
end
...
local function dump_nt_full()
    ...
    for r = 0, 29 do
        local row = {}
        for c = 0, 31 do row[#row + 1] = ciram_u8(r * 32 + c) end
        rows[#rows + 1] = row
    end
    return rows
end
...
local function snapshot()
    return {
        cur_level = u8(CUR_LEVEL), room_id = u8(ROOM_ID),
        game_mode = u8(GAME_MODE), game_sub = u8(GAME_SUB),
        nt = dump_nt_full(), attr = dump_attr(), palram = dump_palram(),
    }
end

-- Press direction button for hold_frames, then settle until in-play or
-- timeout. Returns final room_id (or -1 if not in dungeon).
local function press_dir(button, hold_frames, settle_frames)
    for f = 1, hold_frames do
        local pad = {}; pad[button] = true; pad["P1 " .. button] = true
        safe_set(pad); emu.frameadvance()
    end
    for f = 1, settle_frames or 90 do
        safe_set({}); emu.frameadvance()
        if in_dungeon_play(TARGET_LEVEL) then break end
    end
    if u8(CUR_LEVEL) == TARGET_LEVEL then return u8(ROOM_ID) end
    return -1
end

-- Fully replay a direction sequence after re-warping. Each entry is one of
-- "Up","Down","Left","Right".
local function replay_path(path)
    if not warp_into_level(TARGET_LEVEL) then return false end
    for _, dir in ipairs(path) do
        local rid = press_dir(dir, 600, 240)
...
local function record_room(path_to_here)
    local rid = u8(ROOM_ID)
    local lvl = u8(CUR_LEVEL)
    if lvl ~= TARGET_LEVEL then return false end
    local key = rid
    if visited_paths[key] then return false end
    visited_paths[key] = path_to_here
    local snap = snapshot()
    snap.label = string.format("l%d_r%02x", lvl, rid)
    snap.path = table.concat(path_to_here, ",")
    results[#results + 1] = snap
    table.insert(queue, { rid = rid, path = path_to_here })
    print(string.format("[%d] captured L%d R$%02X via [%s]",
        #results, lvl, rid, snap.path))
    -- Append to a debug log so we can see progress without Lua console.
    local logf = io.open("C:\\Users\\Jake Diggity\\Documents\\GitHub\\FINAL TRY\\RoomRom\\out\\nes_uw_l1_bfs_progress.txt", "a")
    if logf then
        logf:write(string.format("[%d] L%d R$%02X via [%s] frame=%d\n",
            #results, lvl, rid, snap.path, emu.framecount()))
        logf:close()
    end
    return true
end
...
if ok_boot and warp_into_level(TARGET_LEVEL) then
    record_room({})

    local iter = 0
    while #queue > 0 and iter < 200 do
        iter = iter + 1
        local node = table.remove(queue, 1)
        if not replay_path(node.path) then
            print("replay failed for path " .. table.concat(node.path, ","))
            break
        end
        if u8(ROOM_ID) ~= node.rid then
            print(string.format("path drift: expected R$%02X, got R$%02X",
                node.rid, u8(ROOM_ID)))
        end
        for _, dir in ipairs(DIRS) do
            local prev_room = u8(ROOM_ID)
            local new_room = press_dir(dir, 600, 240)
            if new_room >= 0 and new_room ~= prev_room then
                local new_path = {}
                for _, p in ipairs(node.path) do new_path[#new_path + 1] = p end
                new_path[#new_path + 1] = dir
                record_room(new_path)
                -- Walk back via opposite direction so subsequent dirs in
                -- this loop start from node again.
                local back = OPPOSITE[dir]
                local back_room = press_dir(back, 60, 90)
                if back_room < 0 or back_room ~= prev_room then
                    -- Replay from scratch if backtrack drifted.
                    if not replay_path(node.path) then break end
                end
            elseif new_room < 0 then
                -- Left dungeon (e.g. south door from entry). Re-warp + replay.
                if not replay_path(node.path) then break end
            end
        end
    end
...
local function j1(t)
    local s = {}; for i = 1, #t do s[#s + 1] = tostring(t[i]) end
    return "[" .. table.concat(s, ",") .. "]"
end
local function j2(rows)
    local s = {}; for i = 1, #rows do s[#s + 1] = j1(rows[i]) end
    return "[" .. table.concat(s, ",") .. "]"
end
...
f:write('  "level": ', tostring(TARGET_LEVEL), ',\n')
f:write('  "results": [\n')
