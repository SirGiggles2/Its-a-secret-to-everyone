...
.vbi_transpiled:
    ...
    ; (address in Title.lst — .bss region $FFC000+). void(void);
...
;
; HINT now drives scroll splits only.  PCM streaming moved to the SGDK
; XGM Z80 driver (see audio_adapter.c::audio_xgm_init); the Z80 owns
; YM2612 reg $2A and HBlank no longer ticks DAC bytes.
;
...
;==============================================================================
    ...
    include "nes_io.asm"
    ; Stage 2c: asm trampolines that bridge the register-based MoveObject
    ; contract (D2=slot) to the m68k SysV stack ABI used by C.
    include "c_shims.asm"
